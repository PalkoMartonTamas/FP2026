...
szamokOsszege = do
    putStr "Adj meg egy szamot: "
    x1 <- readLn :: IO Int
    putStr "Adj meg meg egy szamot: "
    x2 <- readLn :: IO Int
...
primekOSszege = do
    putStr "Adj meg egy prim szamot: "
    x1 <- readLn :: IO Int
    putStr "Adj meg egy prim szamot: "
    x2 <- readLn :: IO Int
    let primek = primszamok x1 x2
        primOsszeg = sum primek
        reszPrimek = scanl1 (+) primek
    putStrLn ("Prim osszeg: " ++ show primOsszeg)
    putStrLn ("Resz prim osszeg: " ++ show reszPrimek)
...
legtobbValodiOSztokSzam a b = filter (\(szam,oszto) -> oszto == maxOszto) [(i, valodiOsztokSzama i) | i <- [a..b]]
    where
...
ketSzamKozottLegtobbValodiOszto = do
    ...
    let lvosz = legtobbValodiOSztokSzam x1 x2
        losz = snd $ head lvosz
    putStrLn ("A legtobb oszto: " ++ show losz ++ ", az ezzel rendelkezo szamok: ")
...
fiboN = do
    ...
    putStrLn $ show n ++ "-ig a Fibonacci szamok listaja: "
...
primSzamokN = do
    ...
    putStrLn $ show n ++ "-ig a primszamok listaja: "
